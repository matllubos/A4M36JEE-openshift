...
/**
 * Enables messages to be rendered on different pages from which they were set. To produce this behaviour, this class acts as a <code>PhaseListener</code>.
 * <p/>
 * This is performed by moving the FacesMessage objects: <li>After each phase where messages may be added, this moves the messages from the page-scoped FacesContext
 * to the session-scoped session map. <li>Before messages are rendered, this moves the messages from the session-scoped session map back to the page-scoped
 * FacesContext.
 * <p/>
 * Only messages that are not associated with a particular component are ever moved. These are the only messages that can be rendered on a page that is different from
 * where they originated. * To enable this behaviour, add a <code>lifecycle</code> block to your faces-config.xml file. That block should contain a single
 * <code>phase-listener</code> block containing the fully-qualified classname of this file.
 *
 * @author <a href="mailto:jesse@odel.on.ca">Jesse Wilson</a>
 * @version This version have the bug corrected: The bug was: In the same page, the FacesMessage was displayed many times, with this patch, only different messages is
 *          added into new context.
 */
public class MultiPageMessagesSupport implements PhaseListener {
    ...
    /**
     * Remove the messages that are not associated with any particular component from the faces context and store them to the user's session.
     *
