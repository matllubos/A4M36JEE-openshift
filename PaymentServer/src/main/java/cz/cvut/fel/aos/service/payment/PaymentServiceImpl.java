...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Reservation payVisa( final long reservationId, final String password, final String cardName, final long creditCard, final Date validUntil, final int verificationCode ) throws NoSuchReservationException, InvalidPaymentException, SecurityException {
        ...
        log.info( "Accepted credit card '{}' bound to '{}'. There was transferred '{}' money.", new Object[]{ creditCard, cardName, amount } );

        // proveď převod
        reservation.setPaid( reservation.getPaid() + amount );

        log.info( "Reservation with ID '{}' received '{}' money. There is '{}'.", new Object[]{ reservation.getId(), amount, reservation.getPaid() } );
    ...
    public Reservation transferFromReservation( final long reservationIdFrom, final String passwordFrom, final long reservationIdTo, final String passwordTo ) {
        ...
        if ( !reservationFrom.isCanceled() ) { // možné převádět pouze ze zrušených rezervací
            log.info( "Transfer from reservation with ID '{}' was rejected due to the reservation is not cancelled.", reservationIdFrom );
        ...
        if ( reservationTo.isCanceled() ) { // možné převádět pouze na aktivní rezervace
            log.info( "Transfer to reservation with ID '{}' was rejected due to the reservation is cancelled.", reservationIdTo );
        ...
        if ( reservationFrom.getPaid() == 0 ) { // zjisti, jestli zbývají nějaké peníze v rezervaci
            log.info( "Transfer from reservation with ID '{}' to reservation with ID '{}' was rejected due to account is empty.", reservationIdFrom, reservationIdTo );
        ...
        if ( amount == 0 ) { // neplatit již zaplacené rezervace
            log.info( "Payment for reservation with ID '{}' was ignored, reservation is already paid.", reservationIdTo );
            return reservationTo;
        }

        // zaloguj přijetí platební karty
        log.info( "Accepted credit transfer from ID '{}' to ID '{}'. There was transferred '{}' money.", new Object[]{ reservationIdFrom, reservationIdTo, amount } );

        // proveď převod
        reservationFrom.setPaid( reservationFrom.getPaid() - amount );
        reservationTo.setPaid( reservationTo.getPaid() + amount );

        log.info( "Reservation with ID '{}' sent '{}' money. There is '{}'.", new Object[]{ reservationFrom.getId(), amount, reservationFrom.getPaid() } );
        log.info( "Reservation with ID '{}' received '{}' money. There is '{}'.", new Object[]{ reservationTo.getId(), amount, reservationTo.getPaid() } );
